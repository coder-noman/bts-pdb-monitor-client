...
export default function ReportsPage({ onBack }) {
  ...
  async function handleDateSearch(e) {
    ...
    try {
      ...
      const json = await res.json();
      const arr = Array.isArray(json) ? json : json.data || json.routers || json.analytics || [];
  ...
  async function handleExportDate() {
    if (!searchedDate) return;
    setExporting(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/analytics/date/${searchedDate}/excel`);
      if (!res.ok) throw new Error("Failed to export");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `bts-report-${searchedDate}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      alert("Export failed: " + e.message);
        ...
        {data === null && !loading && !error && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-slate-600 text-sm mt-1">Enter date in format: 2026/06/01 and press Enter or Search</p>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-slate-400 text-sm">Loading report for {searchedDate}...</p>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-xl px-4 py-2">
                ...
                <span className="text-indigo-400 font-bold text-sm">{toDisplayDate(searchedDate)}</span>
              </div>
              {data.length > 0 && (
